...
struct ChatroomDetail: View {
    ...
    @Environment(ServerManager.self) var mockServerManager

    var body: some View {
        ChatView(chatroom: chatroom, messages: messages, onSendMessage: { message in
            if !mockServerManager.isServerRunning {
                alertManager.showAlert(message: "Server is not running")
                return
            }
            _ = await ChatManager.shared.sendMessage(chatroomId: chatroom.id, messageRequest: .init(type: .text, content: message))
        })
        .frame(minWidth: 500)
        .task {
