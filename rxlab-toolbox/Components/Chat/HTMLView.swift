        ...
        for element in elements {
            ...
            if element.tag == "text" {
                ...
                for (index, part) in parts.enumerated() {
                    if !part.isEmpty {
                        currentLineElements.append(HTMLElement(
                            tag: "text",
                            text: part,
                            children: [],
                            attributes: [:],
...
struct ElementView: View {
    ...
    private var inlineContent: some View {
        ...
        case "a":
            Link(element.text,
                 destination: URL(string: element.attributes["href"] ?? "") ??
...
#Preview {
    HTMLRenderer(html: """
    <b><i>Welcome to the Rx-Bot Demo</i></b>\n<b>You can use</b> this bot to interact with the Rx-Lab framework.\nClick the menu button to see the available options.<code>Hello</code>
    """)
...
#Preview {
    HTMLRenderer(html: """
    Address: This is a very very very very very long text
    """)
    .frame(maxWidth: 100)
}
